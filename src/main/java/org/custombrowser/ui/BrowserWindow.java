...
import org.custombrowser.jsparser.JsParser;
...
public class BrowserWindow extends Application { // Inheriting from Application class
    ...
    public void stop() throws Exception { // Must override this virtual function
        super.stop();
        System.exit(0); // Forcibly kills all background threads (like HttpClient's pool) when the
                        // window is closed
    }
                ...
                networkManager.fetchPage(url)
                        .thenAccept(html -> {
                            ...
                            try {
                                ...
                                if (dom == null) {
                                    Platform.runLater(
                                            () -> webEngine.loadContent("<h2>Error: Could not parse the HTML.</h2>"));
                                    return;
                                }
                                ...
                                Platform.runLater(() -> {
                                    ...
                                    System.out.println(
                                            "  JS:    Executed " + dom.getElementsByTag("script").size() + " scripts");
                                    webEngine.loadContent(dom.outerHtml());
                                });
                            } catch (Exception e) {
                                Platform.runLater(() -> webEngine
                                        .loadContent("<h2>Error parsing page: " + e.getMessage() + "</h2>"));
                            }
                        })
                        .exceptionally(ex -> {
                            // Catch any hidden errors in the CompletableFuture chain
                            Platform.runLater(() -> webEngine
                                    .loadContent("<h2>Connection/Unexpected Error: " + ex.getMessage() + "</h2>"));
                            return null;
                        });
            });
